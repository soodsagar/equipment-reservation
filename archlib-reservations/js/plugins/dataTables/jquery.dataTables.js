...
(/** @lends <global> */function( $ ) {
	 ...
	 *  @memberof DataTable#oApi
	 */
	function _fnVisibOtelColumnIndex( oSettings, iMatch )
	{
	...
	function _fnScrollDraw ( settings )
	{
		...
		$.each( _fnGetUniqueThs( settings, headerCopy ), function ( i, el ) {
			idx = _fnVisibOtelColumnIndex( settings, i );
	...
	DataTable = function( options )
	{
		...
		this.oApi = this.internal = {
			...
			_fnVisibOtelColumnIndex: _fnVisibOtelColumnIndex,
				...
				if ( match ) {
					switch( match[2] ) {
						...
						case 'visible':
							...
							return [ _fnVisibOtelColumnIndex( settings, idx ) ];
	...
	_api_register( 'column.index()', function ( type, idx ) {
		if ( this.context.length !== 0 ) {
			...
			if ( type === 'fromVisible' || type === 'toData' ) {
				...
				return _fnVisibOtelColumnIndex( ctx, idx );
			}
		}
	} );
	...
	DataTable.ext = _ext = {
		 ...
		 *        return new TabOtelols( { "oDTSettings": oSettings } );
		 *      },
		 *      "cFeature": "T"
		 *    } );
		 */
